...
export function createAroma() {
    ...
    const keyName = 'name';
    const sortNameAroma = nameAroma.sort((nameAroma1, nameAroma2) =>
        nameAroma1[keyName] > nameAroma2[keyName] ? 1 : -1
    );
    const keyMake = 'make';
    const sortMakeAroma = nameAroma.sort((nameAroma1, nameAroma2) =>
        nameAroma1[keyMake] > nameAroma2[keyMake] ? 1 : -1
    );
    ...
    function changeAroma() {
        ...
        const mainNavMenuAroma = document.querySelector(
            '.main-nav-menu--aroma'
        );
        mainNavMenuAroma.classList.add('active');
        mainNavMenuAroma.classList.add('selected');

        const mainNavMenuAromaLogo = document.querySelector(
            '.main-nav--aroma--button-logo'
        );
        ...
        mainNavButtonAromaText.textContent = `${
            sortNameAroma[this.dataset.numberAroma].name
        }. ${sortNameAroma[this.dataset.numberAroma].make}`;
        ...
        let userAroma = {
            section: 'aroma',
            data: {
                name: `${sortNameAroma[this.dataset.numberAroma].name}`,
                make: `${sortNameAroma[this.dataset.numberAroma].make}`,
            },
        };

        postDataToBackend(userAroma);
    }
