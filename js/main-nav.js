...
export function createMainNavMenu(unit) {
    ...
    switch (unit) {
        ...
        case 'cardtext':
            ...
            break;
        case 'aroma':
            listMenuMain = [{ codename: 'aroma', title: '' }];
            break;

        case 'date': {
            listMenuMain = [{ codename: 'date', title: '' }];
            break;
        }
    }
    ...
    if (
        !headerNavItem.classList.contains('created-main-nav') &&
        (unit == 'cardphoto' || unit == 'aroma' || unit == 'date')
    ) {
        for (let i = 0; i < listMenuMain.length; i++) {
            createNavList(
                ...
                mainNav,
                'main',
                listMenuMain[i].codename,
                listMenuMain[i].title,
                unit
            );
        }
